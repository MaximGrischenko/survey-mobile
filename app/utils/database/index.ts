...
export class Database implements IDatabase {
    ...
    public initDB() {
        if(this.database !== undefined) {
            Promise.all(this.tables.map((table) => this.createTable(table)))
                ...
                .catch(reject => {
                  //  this.updateState('Created Error');
            ...
            this.database.then((connect: any) => {
                connect.transaction((txn) => {
                    txn.executeSql(
                        ...
                        [],
                        (tx, res) => {
                          //  this.updateState(`Create table ${table.name} Success`);
    ...
    private syncTable = (table) => {
         return new Promise((resolve, reject) => {
            let api = '';
            let query = '';
            this.updateState({...this.state, pending: true, logger: `Upload data on ${table.name}`});
            switch (table.name) {
                case 'projects': {
